...
function getPageData(page){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            //宣告變數以便迴圈中使用
            let classId,className,classTypeId,classLevelID,sellerID,classStartDate,classPrice,created_at,updated_at;
            //執行迴圈依照JSON長度新增選項
            for (  i = 0 ; i < jsonlength ; i++) {
                classId = data[i]['classId'];
                className = data[i]['className'];
				classTypeId = data[i]['classTypeName'];
				classLevelID = data[i]['classLevel'];
				sellerID = data[i]['sellerID'];
				classStartDate = data[i]['classStartDate'];
				classPrice = data[i]['classPrice'];
				created_at = data[i]['created_at'];
				updated_at = data[i]['updated_at'];
                //動態新增選項
				$('tbody#class_data_body').append(`<tr id="class_Data${i}"></tr>`);
                $(`tr#class_Data${i}`).append(`<td>${classId}</td>`);
                $(`tr#class_Data${i}`).append(`<td>${className}</td>`);
                $(`tr#class_Data${i}`).append(`<td>${classTypeId}</td>`);
                $(`tr#class_Data${i}`).append(`<td>${classLevelID}</td>`);
                $(`tr#class_Data${i}`).append(`<td>${sellerID}</td>`);
                $(`tr#class_Data${i}`).append(`<td>${classStartDate}</td>`);
                $(`tr#class_Data${i}`).append(`<td>${classPrice}</td>`);
                $(`tr#class_Data${i}`).append(`<td>${created_at}</td>`);
                $(`tr#class_Data${i}`).append(`<td>${updated_at}</td>`);
                $(`tr#class_Data${i}`).append(`<td>
												<a class="btn btn-primary" href="./edit_class.php?editclassId=${classId}">查看</a>
												<a class="btn btn-secondary" href="./delete_class.php?deleteclassId=${classId}" onclick="return confirm('確認是否要刪除?')">刪除</a>
											  </td> `);
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            for (  i = 0 ; i < jsonlength ; i++) {
                ...
                $(`tr#class_Data${i}`).append(`<td>
												<a class="btn btn-primary" href="./edit_class.php?editclassId=${classId}">查看</a>
												<a class="btn btn-secondary" href="./delete_class.php?deleteclassId=${classId} onclick="pageChange(this)"">刪除</a>
											  </td> `);
            };
        }
    });
